...
class Android:
	...
	def cc(self):
		if self.is_host():
			...
			s = environ['CC'] if 'CC' in environ else 'clang'
	...
	def cxx(self):
		if self.is_host():
			...
			s = environ['CXX'] if 'CXX' in environ else 'clang++'
			return '%s --target=%s%d' % (s, self.ndk_triplet(), self.api)
	...
	def strip(self):
		if self.is_host():
			...
			return environ['STRIP'] if 'STRIP' in environ else 'llvm-strip'
		if self.ndk_rev >= 23:
